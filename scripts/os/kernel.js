...
function krnOnCPUClockPulse() 
{
	...
	// Simulate active clock in status area by updating every clock pulse
	var dateStatus = document.getElementById("date");
	dateStatus.innerHTML = (currentDate() + " -- " + currentTime()); // Requires Utils.js for currentTime() and currentDate() function.

	// hardwareServices.js required for updating the cpu, ready queue, and memory displays
	
	// Update ready queue display
	updateReadyQueueDisplay();
	
	// Update the memory display if they are showing
	if( _MemoryDisplayCells )
		updateMemoryDisplay();
		
	// Update the filesystem display if they are showing
	if( _FileSystemDisplayCells )
	...
	// If there are no interrupts, and single step status is false, run a cycle
    else if ( _CPU.isExecuting && SINGLE_STEP_STATUS === false )
    {
        _CPU.cycle();
    }    
...
function krnTrace(msg)
{
   ...
   if (_Trace)
   {
      if (msg === "Idle")
      {
         ...
         if (_OSclock % 50 == 0)  // Check the CPU_CLOCK_INTERVAL in globals.js for an 
